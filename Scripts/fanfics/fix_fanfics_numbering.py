...
OBSIDIAN = str(vault_root(__file__))
...
def parse_fanfic_number(name):
    ...
    if sub is not None:
        sub_int = int(sub)
        sub_len = len(sub)
        return main + sub_int / (10**sub_len)
    return float(main)
...
def main():
    ...
    print(
        "Number range: {} - {} (incl. sub: 521.1, 521.2)".format(
            min(all_nums), max(all_nums)
        )
    )
    ...
    if renames_by_old and os.path.isfile(REPORT_PATH):
        ...
        for old_num, (new_num, old_path_str) in renames_by_old.items():
            link_part = old_path_str.replace(".md", "")
            old_link = "[[" + link_part + "]]"
            title_match = re.match(r"Fanfics/\d+\.\s*(.+)", link_part)
            title_only = (
                title_match.group(1).strip()
                if title_match
                else link_part.split(". ", 1)[-1].strip()
            )
