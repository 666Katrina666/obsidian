...
Usage: python Scripts/tools/update_tag_hierarchy.py [folder]
Default folder: Fanfics (relative to vault root).
"""
...
def build_body(files_with_tags, counts):
    ...
    other_dc = [
        ...
        if t.startswith("dc/") and not any(t == x for x, _ in UNIVERSE_DC)
    ]
    for tag in other_dc:
        n = counts[tag]
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="Fanfics",
        help="Folder with .md files (default: Fanfics under vault)",
    )
    args = parser.parse_args()
    folder = args.folder
    vault = str(vault_root(__file__))
    folder_abs = os.path.join(vault, folder) if not os.path.isabs(folder) else folder
    if not os.path.isdir(folder_abs):
        print(f"Error: folder not found: {folder_abs}")
        return 1
    output_path = os.path.join(vault, OUTPUT_FILE)
...
"""
    ...
    with open(output_path, "w", encoding=ENCODING) as f:
        ...
        f.write(tail)
    print(
        f"Updated {OUTPUT_FILE} (scanned {files_with_tags} files, {len(counts)} unique tags)"
    )
    return 0
