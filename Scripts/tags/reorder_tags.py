# -*- coding: utf-8 -*-
"""
Reorder first-line tags in .md files according to hierarchy (Иерархия_тегов.md).
Usage: python Scripts/tags/reorder_tags.py <folder>
"""
...
def parse_tag_line(line: str) -> tuple[str | None, list[str]]:
    ...
    return (prefix, list(dict.fromkeys(tags)))
...
def reorder_tags_in_file(path: Path) -> bool:
    ...
    _prefix, tags = parse_tag_line(first)
    if not tags:
        return False
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python Scripts/tags/reorder_tags.py <folder>")
        sys.exit(1)
