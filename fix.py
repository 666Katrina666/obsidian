...
# Функция для удаления ссылок Markdown [текст](ссылка) -> текст
def remove_links(line):
    # Паттерн для поиска ссылок в формате [текст](ссылка)
    pattern = r'\[([^\]]+)\]\([^\)]+\)'
    # Заменяем ссылку на только текст из квадратных скобок
    ...
    with open(input_path, encoding='utf-8') as infile, open(output_path, "w", encoding="utf-8") as outfile:
        for lineno, line in enumerate(infile):
            if lineno < 7:
                ...
                if line.strip():
                    # Сначала удаляем ссылки, потом исправляем абзацы
...
if __name__ == "__main__":
    ...
    def print_usage():
        ...
        print("  python fix.py START-END        # e.g., 324-333 (uses Fanfics/START..END.md)")
        print("  python fix.py START END        # e.g., 324 333 (uses Fanfics/START..END.md)")
        print("  python fix.py N                # e.g., 324 (uses Fanfics/324.md)")
